...
void MeasureAlgorithms(Vector<geom::Point2D> &points) {
    ...
    auto duracaoGrahamMerge = std::chrono::duration_cast<std::chrono::duration<double>>(end - start);

    start = std::chrono::high_resolution_clock::now();
    geom::ConvexHull::GrahamScan(copyPoints2, convex2, geom::Utils::InsertionSort);

    end = std::chrono::high_resolution_clock::now();
    auto duracaoGrahamInsertion = std::chrono::duration_cast<std::chrono::duration<double>>(end - start);

    start = std::chrono::high_resolution_clock::now();
    geom::ConvexHull::GrahamScan(copyPoints3, convex3, geom::Utils::BucketSort);

    end = std::chrono::high_resolution_clock::now();
    auto duracaoGrahamBucket = std::chrono::duration_cast<std::chrono::duration<double>>(end - start);

    start = std::chrono::high_resolution_clock::now();
    geom::ConvexHull::JarvisMarch(copyPoints4, convex4);

    end = std::chrono::high_resolution_clock::now();
    auto duracaoJarvis = std::chrono::duration_cast<std::chrono::duration<double>>(end - start);
...
int main(int argc, char *argv[]) {
    ...
    if (useRandom) {
        ...
        if (geom::Utils::IsPolygon(points)) {
            ...
            std::cout << "Erro: O conjunto de pontos gerados não forma um polígono válido" << std::endl;
        }
    }
    else {
        while (run) {
            if (useInterative) {
                ...
                std::cout << "Digite: 'fecho caminho/arquivo/entrada' ou apenas 'caminho/arquivo/entrada'.\nPressione ENTER ou digite 'EXIT', 'exit' para fechar." << std::endl;
            ...
            if (!file.is_open()) {
                std::cout << "Erro: Não foi possível ler o arquivo '" << filePath << "'" << std::endl;
                return EXIT_FAILURE;
            }
            ...
            if (geom::Utils::IsPolygon(points)) {
                ...
                std::cout << "Erro: O conjunto de pontos não forma um polígono válido" << std::endl;
                return EXIT_FAILURE;
            }
