...
namespace geom
{
    ...
    void
    Utils::CreateRandomPoints(int amount, int min, int max, Vector<Point2D>& points)
    {
    ...
    void Utils::BucketSort(Vector<Point2D>& vector)
    {
        ...
        Point2D** bucketsPoints =
            new Point2D*[numBuckets]; // Array de ponteiros para arrays (estouro da piha
                                      // quando vector.size era maior que 2320)
    ...
    void Utils::MergeSortedSubarrays(Vector<Point2D>& subvector,
                                     const Point2D&   ref,
                                     int              left,
                                     int              middle,
                                     int              right)
    ...
    void Utils::MergeSortHelper(Vector<Point2D>& subvector,
                                const Point2D&   ref,
                                int              left,
                                int              right)
    {
        if (left < right)
        {
            int middle =
                left + (right - left) /
                           2; // Evita overflow para valores de left e right grandes
    ...
    {
        // Cross product a x b
        double magnitude = (a.GetX() - ref.GetX()) * (b.GetY() - ref.GetY()) -
                           (a.GetY() - ref.GetY()) * (b.GetX() - ref.GetX());
